...
def llm_pre_res(llm_res_path, gold_res_path):
    ...
    for i, (key, value) in enumerate(llm_res_dict.items()):
        ...
        try:
            if len(pre_res) == len(gold_res_dict[key]):
                llm_pred += pre_res
                gold_res += [s.replace("B-地名", "地名").replace("I-地名", "地名").replace("B-役者", "役者").replace("I-役者", "役者").replace("B-演目", "演目").replace("I-演目", "演目").replace("B-替名", "替名").replace("I-替名", "替名") for s in gold_res_dict[key]]
        except Exception as e:
            print("key:", key)
            print("true_res:", true_res)
            
    
    labels = sorted(set(llm_pred + gold_res) - {"O"})
...
def main():
    llm_res_path = "/root/GMNER/saved_model/claude_ner_res_4_1_tag_without_hint_repair.txt"
